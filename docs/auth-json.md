...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| `auth_mode` | `string` | ChatGPT OAuth 登录固定为 `chatgpt`。 |
| `OPENAI_API_KEY` | `string \| null` | OAuth 登录时为 `null`。 |
...
添加 OAuth 账号时，可以粘贴完整 `auth.json`，也可以只粘贴 `refresh_token`。应用会先刷新凭据，再按上述标准结构保存。
