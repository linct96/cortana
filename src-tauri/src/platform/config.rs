use super::state::{ConfigDiagnostic, ConfigFile};
...
pub(crate) fn validate_toml(content: &str) -> Vec<ConfigDiagnostic> {
...
pub(crate) fn format_toml(content: &str, filename: &str) -> Result<String, String> {
...
pub(crate) fn parse_toml(content: &str, filename: &str) -> Result<toml::Value, String> {
    toml::from_str(content).map_err(|error| format!("{filename} 格式错误：{error}"))
}

pub(crate) fn validate_json_object(content: &str, display_name: &str) -> Vec<ConfigDiagnostic> {
...
pub(crate) fn format_json_object(content: &str, display_name: &str) -> Result<String, String> {
    serde_json::to_string_pretty(&parse_json_object(content, display_name)?)
        .map_err(|error| error.to_string())
}

pub(crate) fn parse_json_object(content: &str, display_name: &str) -> Result<Value, String> {
...
pub(crate) fn byte_offset_to_utf16(content: &str, offset: usize) -> usize {
    content
        .char_indices()
