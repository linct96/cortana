use super::types::CliEnvironment;
use crate::{
    platform::{db, state::AppState},
...
pub(crate) async fn is_codex_cli_available(state: State<'_, AppState>) -> Result<bool, String> {
...
pub(crate) async fn is_claude_cli_available(state: State<'_, AppState>) -> Result<bool, String> {
    let state = state.inner().clone();
...
pub(crate) async fn is_antigravity_cli_available(
...
pub(crate) async fn is_grok_cli_available(state: State<'_, AppState>) -> Result<bool, String> {
    let state = state.inner().clone();
...
pub(crate) async fn get_codex_cli_environment(
    state: State<'_, AppState>,
...
pub(crate) async fn get_claude_cli_environment(
    state: State<'_, AppState>,
) -> Result<CliEnvironment, String> {
    let state = state.inner().clone();
...
pub(crate) async fn get_antigravity_cli_environment(
    state: State<'_, AppState>,
) -> Result<CliEnvironment, String> {
    let state = state.inner().clone();
...
pub(crate) async fn get_grok_cli_environment(
    state: State<'_, AppState>,
) -> Result<CliEnvironment, String> {
    let state = state.inner().clone();
...
pub(crate) fn get_terminal_app(state: State<'_, AppState>) -> Result<String, String> {
...
pub(crate) fn codex_candidates(state: &AppState) -> Vec<PathBuf> {
    let home = state
        .default_codex_home
        .parent()
...
pub(crate) fn find_codex(state: &AppState) -> Option<(PathBuf, String)> {
...
pub(crate) fn grok_home(state: &AppState) -> PathBuf {
...
pub(crate) fn grok_candidates(state: &AppState) -> Vec<PathBuf> {
...
pub(crate) fn codex_command(candidate: &Path, arguments: &[&str]) -> Command {
    #[allow(unused_mut)]
...
mod tests {
    use super::{
        antigravity_install_method, claude_install_method, codex_launcher_script,
        normalize_install_method, parse_claude_version, parse_cli_version, parse_codex_version,
        parse_doctor_environment, parse_grok_version,
    };
    use serde_json::json;
    use std::path::Path;
