use crate::platform::{db, state::AppState};
...
pub(crate) struct ModelPricing {
    ...
    pub(crate) input_cost_per_million: String,
    pub(crate) output_cost_per_million: String,
    pub(crate) cache_read_cost_per_million: String,
    pub(crate) cache_write_cost_per_million: String,
}

#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(crate) struct ModelsDevPricing {
    provider: String,
    model_id: String,
    display_name: String,
    release_date: String,
...
pub(crate) fn list_model_pricing(state: State<'_, AppState>) -> Result<Vec<ModelPricing>, String> {
    let connection = db::open_database(&state)?;
    list_model_pricing_from(&connection)
}

pub(crate) fn save_model_pricing(
...
pub(crate) fn delete_model_pricing(
    state: State<'_, AppState>,
    model_id: String,
) -> Result<(), String> {
    let connection = db::open_database(&state)?;
...
pub(crate) async fn fetch_models_dev_pricing() -> Result<Vec<ModelsDevPricing>, String> {
...
mod tests {
    use super::{
        estimated_cost, list_model_pricing_from, parse_models_dev_pricing, save_model_pricing_to,
        ModelPricing,
    };
