use crate::{
    features::accounts::{get_profile_summary_for_product, list_profiles_for_product},
    platform::{
        config,
        ...
        local_web,
        state::{
            now_millis, AccountProduct, AntigravityModelQuota, AntigravityQuota,
            AntigravityQuotaBucket, AntigravityQuotaGroup, AppState, AppStatus, AuthState,
            ConfigDiagnostic, ConfigFile, OAuthTokenResponse, ProfileSummary,
            ANTIGRAVITY_OAUTH_CLIENT_ID, ANTIGRAVITY_OAUTH_CLIENT_SECRET,
            ANTIGRAVITY_OAUTH_TOKEN_URL, ANTIGRAVITY_OAUTH_USERINFO_URL,
        },
    },
};
...
pub(crate) fn get_antigravity_config(state: State<'_, AppState>) -> Result<ConfigFile, String> {
...
pub(crate) fn validate_antigravity_config(content: String) -> Vec<ConfigDiagnostic> {
    config::validate_json_object(&content, "Antigravity settings.json")
}

pub(crate) fn format_antigravity_config(content: String) -> Result<String, String> {
    config::format_json_object(&content, "Antigravity settings.json")
}

pub(crate) fn save_antigravity_config(
...
pub(crate) fn app_status(app: &tauri::AppHandle, state: &AppState) -> Result<AppStatus, String> {
    ...
    Ok(AppStatus {
        ...
        auth_state: AuthState {
            ...
            message: if managed {
                "显示上次由 Cortana 切换的 Antigravity CLI OAuth 账号。"
            } else {
...
pub(crate) fn upsert_oauth_profile(
    ...
    let existing_id = transaction
        .query_row(
            "SELECT id FROM accounts WHERE product = 'antigravity' AND account_type = 'oauth' AND (account_id = ?1 OR email = ?2 COLLATE NOCASE) LIMIT 1",
    ...
    if transaction
        .execute(
            "UPDATE accounts SET account_type = 'oauth', api_base_url = NULL, account_id = ?1, email = ?2, alias = CASE WHEN ?3 = '' THEN alias ELSE ?3 END, auth_json = ?4, updated_at = ?5 WHERE id = ?6 AND product = 'antigravity'",
...
pub(crate) fn switch_profile(
    ...
    let auth_json = connection
        .query_row(
            "SELECT auth_json FROM accounts WHERE id = ?1 AND product = 'antigravity' AND account_type = 'oauth'",
            params![profile_id],
            |row| row.get::<_, String>(0),
        )
        .optional()
        .map_err(database_error)?
        .ok_or_else(|| "Antigravity OAuth 账户不存在。".to_string())?;
...
pub(crate) fn update_alias(
    ...
    let changed = connection
        .execute(
            "UPDATE accounts SET alias = ?1, updated_at = ?2 WHERE id = ?3 AND product = 'antigravity' AND account_type = 'oauth'",
...
pub(crate) fn refresh_profile_usage(
    ...
    let (auth_json, cached_quota, current_plan) = connection
        .query_row(
            "SELECT auth_json, antigravity_quota_json, plan_type FROM accounts WHERE id = ?1 AND product = 'antigravity' AND account_type = 'oauth'",
            params![profile_id],
            |row| {
                Ok((
                    row.get::<_, String>(0)?,
...
mod tests {
    use super::{
        build_auth_json, build_runtime_credential, config_path, format_antigravity_config,
        import_current_profile, parse_available_models, parse_quota_summary,
        read_antigravity_config, resolve_plan_type, save_antigravity_config_at,
        write_file_atomically, AntigravityAuth, AntigravityToken, AppState, LoadCodeAssistResponse,
        Mutex, OAuthTokenResponse, PathBuf, Uuid, Value,
    };
    use serde_json::json;
    use std::{fs, sync::Arc};
    ...
    #[test]
    fn rejects_importing_non_oauth_antigravity_accounts() {
        let state = AppState {
        ...
        assert_eq!(
            import_current_profile(&state, None).unwrap_err(),
            "请使用浏览器 OAuth 添加 Antigravity 账号。"
        );
    }
