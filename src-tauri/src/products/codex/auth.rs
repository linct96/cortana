use super::config::{
    auth_path, extract_api_key, extract_refresh_token, read_optional_file, restore_optional_file,
    write_auth_json_atomically,
};
use crate::{
    features::{
        accounts::{
            oauth::{build_codex_auth_json, decode_jwt_claims, refresh_oauth_token_detailed},
            resolve_auth_state,
        },
        environment,
    },
    platform::{
        ...
        state::{now_millis, AppState, ACCOUNT_TYPE_OAUTH, ACCOUNT_TYPE_RELAY},
    },
};
use chrono::Utc;
...
pub(crate) const CODEX_RELAY_API_KEY_ENV: &str = "CORTANA_CODEX_RELAY_API_KEY";
// ponytail: 账号量很小；出现可测量的跨账号等待时再拆为每账号锁。
static CODEX_AUTH_REFRESH_LOCK: Mutex<()> = Mutex::new(());

pub(crate) struct CodexApiError {
    pub(crate) message: String,
    pub(crate) unauthorized: bool,
    pub(crate) authentication_invalidated: bool,
}
...
pub(crate) fn authentication_invalidated(status: u16, body: &str) -> bool {
    if status != 401 {
        return false;
    }
...
pub(crate) fn open_codex_cli_with_profile_internal(
    state: &AppState,
    profile_id: &str,
) -> Result<(), String> {
    ...
    environment::open_codex_cli(state, &environment, &arguments)
}
...
pub(crate) fn codex_token_needs_refresh(auth_json: &str, now: i64) -> Result<bool, String> {
...
pub(crate) fn codex_auth_needs_refresh(
    auth_json: &str,
...
pub(crate) fn ensure_fresh_codex_auth(
    state: &AppState,
    profile_id: &str,
    rejected_auth: Option<&str>,
...
pub(crate) fn mark_codex_oauth_invalidated(
...
pub(crate) fn with_codex_auth_retry<T>(
    state: &AppState,
    profile_id: &str,
...
pub(crate) fn finish_codex_request<T>(
    state: &AppState,
    profile_id: &str,
...
pub(crate) fn persist_codex_cli_auth(
    state: &AppState,
    connection: &Connection,
    profile_id: &str,
    auth_json: &str,
...
pub(crate) fn codex_cli_config_override(key: &str, value: &str) -> String {
    format!("{key}={}", toml_edit::Value::from(value))
}

pub(crate) fn codex_relay_cli_options(
    api_key: String,
    api_base_url: &str,
