use super::{
    accounts::*,
    codex::{normalize_api_base_url, write_file_atomically},
    db::*,
    local_web, *,
};
...
pub(super) const CLAUDE_OAUTH_CLIENT_ID: &str = "9d1c250a-e61b-44d9-88ed-5944d1962f5e";
pub(super) const CLAUDE_OAUTH_CALLBACK_URL: &str = "http://localhost:54545/callback";
pub(super) const CLAUDE_OAUTH_SCOPE: &str =
...
pub(super) struct ClaudeOAuthTokenResponse {
...
pub(super) fn clear_active_profile(state: &AppState, profile_id: &str) -> Result<(), String> {
    let connection = open_database(state)?;
...
pub(super) fn credential_is_renewable(auth_json: &str) -> bool {
...
#[tauri::command]
pub(super) fn get_claude_config(state: State<'_, AppState>) -> Result<ConfigFile, String> {
...
#[tauri::command]
pub(super) fn save_claude_config(
...
#[tauri::command]
pub(super) fn validate_claude_config(content: String) -> Vec<ConfigDiagnostic> {
    config::validate_json_object(&content, "Claude settings.json")
}

#[tauri::command]
pub(super) fn format_claude_config(content: String) -> Result<String, String> {
